...
# `epicandi/epicandi`: Contributing guidelines

Hi there!
Thanks for taking an interest in improving epicandi/epicandi.

This page describes the recommended nf-core way to contribute to both epicandi/epicandi and nf-core pipelines in general, including:

- [General contribution guidelines](#general-contribution-guidelines): common procedures or guides across all nf-core pipelines.
- [Pipeline-specific contribution guidelines](#pipeline-specific-contribution-guidelines): procedures or guides specific to the development conventions of epicandi/epicandi.
...
- [ ] Check whether a GitHub [issue](https://github.com/epicandi/epicandi/issues) about your idea already exists. If an issue does not exist, create one so that others are aware you are working on it.
- [ ] [Fork](https://help.github.com/en/github/getting-started-with-github/fork-a-repo) the [epicandi/epicandi repository](https://github.com/epicandi/epicandi) to your GitHub account.
...
You can contribute to epicandi/epicandi without installing a local development environment on your machine by using [GitHub Codespaces](https://github.com/codespaces).
...
To get started, open the repository in [Codespaces](https://github.com/epicandi/epicandi/codespaces).
...
nf-core semi-standardises how you write code and other contributions to make the epicandi/epicandi code and processing logic more understandable for new contributors and to ensure quality.
...
- Tools used by a new step must come from `nf-core/modules` whenever an upstream module exists. If none exists or it does not cover the desired flags, write a local module under `modules/local/` following the same conventions (tag, label, conda+container, topic versions, stub, meta.yml).
- Per-step `withName` overrides live in `conf/modules.config`; do not pass `ext.args` inline in the process call.
- Python helpers are installed under `bin/` and invoked from local modules via the shared `nf-python-helpers` conda env (Python 3.12 + pandas + numpy + biopython).
- For SNP calling: Illumina / hybrid use GATK4 HaplotypeCaller (tier HIGH), Nanopore-only uses Clair3 (tier MEDIUM). Mixing tiers in a single joint-genotype run is not supported (different error models). See `docs/gatk4/README.md` for the design rationale.
- For phylogeny: IQ-TREE2 with `-m GTR+G+ASC` on the SNP-only alignment from vcf2phylip + snp-sites; snp-dists runs separately on the full alignment to give absolute pairwise SNP distances. The UPGMA network (NetworkX) consumes the snp-dists matrix, not the IQ-TREE topology, and colours nodes by samplesheet `batch`.
