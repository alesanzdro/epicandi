process GATK4_HAPLOTYPECALLER {
    ...
    // Local patch: in this pipeline mask.bed (rRNA, repeats, low-complexity)
    // is passed through the `intervals` input slot, but its meaning is
    // EXCLUDE, not include. The upstream nf-core module wires it as
    // --intervals (a positive scope) which made HaplotypeCaller call variants
    // ONLY inside the bad regions. Flip to --exclude-intervals so the mask
    // actually excludes those regions.
    def interval_command = intervals ? "--exclude-intervals ${intervals}" : ""
